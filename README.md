...
Каталог `tests/scenarios/` содержит фиксированный приёмочный мир (один сектор,
четыре системы, двенадцать поселений и четыре ресурса), проверки точных
товарного и денежного балансов, режимов LOD, детерминизма worker-потоков и
эквивалентности save/load. Отдельная команда профиля измеряет 100-летний прогон,
память, размеры сохранения и истории, сериализацию и ускорение 2/4/8 потоков;
подробные допуски и состав отчёта описаны в `tests/scenarios/README.md`.
